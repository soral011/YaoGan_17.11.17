...
void led_control::sendmsg()
{
    ...
    QByteArray filedata,tmpdata,filename;
    filedata=file.readAll();
//    qDebug()<<"filedata:"<<filedata.toHex();
    file.close();
    filename = "play.lst";
    tmpdata.append(0x02);
    tmpdata.append('0');
    ...
    tmpdata.append(0x2b);
    int len;
    len = 0;//filedata.count();
    tmpdata.append((len>>24)&0xff);
    tmpdata.append((len>>16)&0xff);
    tmpdata.append((len>>8)&0xff);
    tmpdata.append((len>>0)&0xff);
    tmpdata.append(filedata);
...
//    QDataStream out(&block,QIODevice::WriteOnly);
    ...
    data = tmpdata.data();
    qDebug()<<"tmp count"<<tmpdata.count();

    unsigned short crc=getCRC16((uchar*)(data+1),tmpdata.count()-1);
...
//    tmpdata.replace(0x1b,QByteArray::fromHex("1B00"));
//    tmpdata.replace(0x02,QByteArray::fromHex("1BE7"));
//    tmpdata.replace(0x03,QByteArray::fromHex("1BE0"));



    tmpdata[0] = 0x02;
    tmpdata.append((crc>>8)&0xff);
    tmpdata.append(crc&0xff);
    tmpdata.append(0x03);

//    qDebug()<<"msg:"<<tmpdata.toHex()<<",count:"<<tmpdata.count();
    tcpSocket.write(tmpdata);
}
...
//    return ((crch<<8) |  crcl);
    ...
    int i;
    for (i = 0; i < len; i++)
        crc = crc_table[((crc >> 8) ^ ptr[i]) & 0xFF] ^ (crc << 8);
    return crc;
